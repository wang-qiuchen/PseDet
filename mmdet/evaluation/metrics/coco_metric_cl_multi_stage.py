...
class CocoMetricCLMultiStage(BaseMetric):
    ...
    def __init__(self,
                 class_group: list=[40,40],# 44 is [40,40]; 71 is [70,10]; 422 is [40,20,20]
    ...
    def gt_to_coco_json(self, gt_dicts: Sequence[dict],
        ...
        for idx, gt_dict in enumerate(gt_dicts):
            ...
            labels = gt_dict['anns']['labels'].tolist()
            bboxes = gt_dict['anns']['bboxes'].tolist()
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        for index,num_classes in enumerate(self.class_group):
            print('\n\ncomputing_stage%s...'%(index+1))
            class_label_start = sum(self.class_group[:index])
            class_label_end = class_label_start + num_classes
            stage_result = []
            for gt_all,stage_preds in results:
                stage_gt=deepcopy(gt_all)
                labels = stage_gt['anns']['labels']
                indices = torch.where((labels > (class_label_start-1)) & (labels <= (class_label_end-1)))
                stage_gt['anns']['labels'] = stage_gt['anns']['labels'][indices]
                stage_gt['anns']['bboxes'] = stage_gt['anns']['bboxes'][indices]
                stage_result.append((stage_gt, stage_preds))
            result_dict = self.compute_metrics_stage_by_stage(stage_result)
            for key in result_dict.keys():
                new_key = f'stage{index+1}_{key}'
                eval_results[new_key] = result_dict[key]  
    ...
    def compute_metrics_stage_by_stage(self, results: list) -> Dict[str, float]:
        """Compute the metrics from processed results.
