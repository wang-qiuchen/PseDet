...
class AdaptiveSampleAssigner(BaseAssigner):
    ...
    def assign(self,
        ...
        priors_c = (pred_instances.priors[:,:2] + pred_instances.priors[:,2:4] ) / 2.0
        priors_stride = (pred_instances.priors[:,2:4] - pred_instances.priors[:,:2] ) / 2.0
        priors = torch.cat((priors_c,priors_stride),dim=-1)
        ...
        pairwise_ious = self.iou_calculator(valid_decoded_bbox,gt_bboxes[:, :4]).detach()
        ...
        cls_cost = F.binary_cross_entropy(pred, target, reduction='none') * focal_weight
