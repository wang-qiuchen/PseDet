...
class DetTTAModelScalesKmeans(BaseTTAModel):
    ...
    def __init__(self, tta_cfg=None, **kwargs):
        ...
        self.scores_all = torch.zeros(self.tta_cfg.buffer_size)    
    ...
    def _merge_single_sample(self, data_samples: List[DetDataSample]) -> DetDataSample:
        ...
        det_bboxes = det_bboxes; det_labels = merged_labels[keep_idxs]; _det_bboxes = det_bboxes.clone()
        bboxes = _det_bboxes[:, :-1]
        scores = _det_bboxes[:, -1]
        labels = det_labels
        
        if self.tta_cfg.get('kmeans',False):
            thrs = torch.zeros_like(scores)
            for label in torch.unique(labels):
                label = int(label)
                scores_add = (scores[labels == label])
                num_buffers = len(self.scores_all[label])
                scores_new= torch.cat([scores_add.float().to(self.scores_all.device), self.scores_all[label].float()])[:num_buffers]
                self.scores_all[label] = scores_new

                kmeans = KMeans(n_clusters=2)
                labels_in = kmeans.fit_predict(scores_new.reshape(-1, 1))
                
                cluster_1 = scores_new[labels_in == 0]
                cluster_2 = scores_new[labels_in == 1]
                min1 = cluster_1.min();min2 = cluster_2.min()
                thr = min1 if min1 > min2 else min2
                thrs[labels == label] = thr
            valid_mask = scores > thrs
